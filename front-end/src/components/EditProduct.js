...
export default class EditProduct extends Component {
    ...
    componentDidMount = async () => {
        ...
        await fetch(`http://52.91.20.141:4000/v1/product/${idProduct}`, {
            crossDomain: true
        })
            .then(res => res.json())
            .then(async res => {
                ...
                for (let i = 0; i < res.rows[0].tags.split(",").length; i++) {
                    await fetch(`http://52.91.20.141:4000/v1/tag/${res.rows[0].tags.split(",")[i]}`, { crossDomain: true })
                        .then(res => res.json())
        ...
        await fetch(`http://52.91.20.141:4000/v1/category`, { crossDomain: true })
            .then(res => res.json())
            .then(results => {
    ...
    handleKeyDownTag = async event => {
        if (event.key === "Enter") {
            ...
            if (!this.state.tagSuggestions.find(x => x.nameOfTag === this.state.tagSearchInput) && this.state.tagSearchInput !== "") {
                ...
                await fetch(`http://52.91.20.141:4000/v1/tag/name/${this.state.tagSearchInput}`, {
                    crossDomain: true
                })
                    .then(res => res.json())
                    .then(results => {
    ...
    onSubmit = (event) => {
        ...
        let idProduct = parseInt(this.props.match.params.id);
        axios.put(`http://52.91.20.141:4000/v1/product/${idProduct}`, {
            ...editedProduct
        },
            {
                headers: {
                                ...
                                <div className={styles.row}>
                                    <div className={styles.row}>
                                        ...
                                        <img
                                            ...
                                            height="100"
                                            alt="Product"
                                            src={(this.state.imageUrl !== null) ? `http://52.91.20.141:4000/${this.state.imageUrl}` : `http://52.91.20.141:4000/${this.state.images}`}
