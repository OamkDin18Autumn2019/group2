...
const SearchBar = props => {
  return (
    <form className={styles.SearchBox} onSubmit={props.searchSubmitHandler}>
      ...
      />
      <button type="submit" className={ButtonStyles.IconButtons}>
        {" "}
        <img
          src={SearchLogo}
          alt="decorative"
          className={classNames(styles.SearchLogo, styles.Icons)}
        />{" "}
      </button>
    </form>
  );
...
export default class Header extends React.Component {
  ...
  DropDownClickHandler = event => {
    this.setState({
      DropDownMenuButton:
        this.state.DropDownMenuButton === styles.ArrowDownCollapsed
          ? styles.ArrowDownExpanded
          : styles.ArrowDownCollapsed
    });
    this.setState({
      DropDownMenu:
        this.state.DropDownMenu === styles.DropDownMenuCollapsed
          ? styles.DropDownMenuExpanded
          : styles.DropDownMenuCollapsed
    });
    this.setState({
      ArrowState: this.state.ArrowState === ArrowUp ? ArrowDown : ArrowUp
    });
    this.setState({
      NavbarClass:
        this.state.NavbarClass === styles.mainNav
          ? styles.mainNavExpanded
          : styles.mainNav
    });
  };
  ...
  searchSubmitHandler = event => {
    ...
    this.props.history.push(
      "/search?q=" + encodeURIComponent(this.state.searchInput)
    );
  };
